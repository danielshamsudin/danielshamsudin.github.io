...
let logoSrc ="./images/unifi.png"
    ...
    function timeout() {
        ...
        alert("You had earned " + best + " score.");
...
loadAjaX("https://fuyoh-ads.com/game_web/campaign_game/settings?activity_id=4", function(){
...
function initGameSetting()
{
    ...
    const medal = {
        ...
        if(state.current == state.over && score.value >= 20 && score.value < 30){
        ctx.drawImage(sprite, this.sX, this.sY-46, this.w, this.h, this.x, this.y, this.w, this.h);
        }
        if(state.current == state.over && score.value >= 30 && score.value < 40){
...
console.log("detect width "+windowWidth)
    if(windowWidth <="1080"||windowWidth >="1000") //match with css     @media only screen and (min-width: 1080px) and (min-height: 1920px )
    {
        xRatio = 3.2;
        yRatio = 3.4;
        wRatio = 2.4;
