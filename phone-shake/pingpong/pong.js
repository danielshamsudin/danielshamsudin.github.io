  ...
  DeviceMotionEvent.requestPermission()
    .then((permissionState) => {
      if (permissionState === "granted") {
        ...
        alert("Please approve the device motion to proceed the game.");
      }
    })
    .catch(console.error);
} else {
  // handle regular non iOS 13+ devices
  // alert("Approve the device motion to proceed the game.");
}
...
function movePaddle(evt) {
  if (window.innerWidth >= window.innerHeight) {
    ...
    alert(
      "Please make sure you opened auto-rotate function and turn your device in landscape mode for best experience."
    );
  }
}
...
function update() {
  ...
  if (
    ...
    ball.y + ball.radius >= canvas.height - ball.radius
  ) {
    ball.velocityY = -ball.velocityY;
    wall.play();
  }
